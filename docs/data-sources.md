...
| **CEDEN fecal indicator bacteria** (county weekly sampling) | CKAN datastore API, [dataset](https://data.ca.gov/dataset/surface-water-fecal-indicator-bacteria-results); resources: 2020+ `15a63495…`, 2010–20 `04d98c22…`, pre-2010 `1d333989…` | Linda Mar #5 2000→, #6 1998–2008, San Pedro Creek 2015→; weekly; includes CollectionTime | `ceden_raw` (via `pipeline/fetch.py`) | ✅ live |
...
| **NOAA CO-OPS tide predictions** — SF station 9414290 (phase leads Pacifica by minutes; deterministic → exact retroactively and forecastable ahead) | CO-OPS API, hourly predictions | 2000 → present (full LM5 era; extendable any range) | `tide_hourly` | ✅ live |
